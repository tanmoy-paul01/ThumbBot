...
const MyGeneration = () => {
    ...
    const handleDelete = async (id: string) => {
        try {
            const confirm = window.confirm('Are you want to delete this thumbnail?')
            if (!confirm) return;
                            ...
                            <div key={thumb._id} onClick={() => navigate(`/generate/${thumb._id}`)} className="mb-8 group relative cursor-pointer rounded-2xl bg-white/6 border border-white/10 transition shadow-xl break-inside-avoid">
                                ...
                                <div
                                    ...
                                    {thumb.image_url ? (
                                        ...
                                        <div className="w-full flex h-full items-center justify-center text-sm text-zinc-400">
                                ...
                                <div onClick={(e) => e.stopPropagation()} className="absolute bottom-2 right-2 max-sm:flex sm:hidden group-hover:flex gap-1.5">
                                    <TrashIcon onClick={(e)=>e.stopPropagation()} className="size-6 bg-black/50 p-1 rounded hover:bg-pink-600 transition-all" />
